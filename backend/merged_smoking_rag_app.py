...
try:
    legacy_dir = Path(__file__).resolve().parents[1] / "legacy" / "shs_chatbot_pinecone"
...
DB_PATH = LOG_DIR / "conversation_memory.sqlite"
...
def is_shs_relevant_query(query: str) -> bool:
    """
    Return True when query is likely related to smoking/SHS/cessation.
    Unrelated queries should not block on heavy RAG initialisation.
    """
    q = (query or "").lower()
    shs_terms = [
        "smoke", "smoking", "secondhand", "second hand", "shs", "ets", "thirdhand",
        "third hand", "nicotine", "cigarette", "tobacco", "vape", "vaping",
        "quit", "cessation", "asthma", "sids", "lung", "passive smoke",
        "child", "baby", "infant", "parent", "caregiver",
    ]
    return any(term in q for term in shs_terms)
...
def main() -> None:
    ...
    if "rag_bootstrap_done" not in st.session_state:
        with st.spinner("Initializing knowledge base (RAG) ..."):
            try:
                _boot_rag = get_rag_layer()
                st.session_state.rag_is_ready = _boot_rag.is_ready
                st.session_state.rag_error = _boot_rag.error_message or ""
            except Exception as e:
                st.session_state.rag_is_ready = False
                st.session_state.rag_error = str(e)
                log_pipeline_error(f"RAG bootstrap failed: {e}")
        st.session_state.rag_bootstrap_done = True
    ...
    if "session_id" not in st.session_state:
        st.session_state.session_id = hashlib.md5(
            f"{datetime.utcnow().isoformat()}-{user_age}".encode("utf-8")
        ).hexdigest()[:16]
    if "context_manager" not in st.session_state:
        st.session_state.context_manager = ContextManager(recent_min=4, recent_max=8)
    if "summarizer" not in st.session_state:
        st.session_state.summarizer = ConversationSummarizer()
    if "token_budgeter" not in st.session_state:
        st.session_state.token_budgeter = TokenBudgeter(max_input_tokens=1800)
    if "summary_last_message_count" not in st.session_state:
        st.session_state.summary_last_message_count = 0
    ...
    retrieval_manager = RetrievalManager(mem_store)
    prompt_builder = PromptBuilder(MEDICAL_PREFIX.strip())
    context_manager: ContextManager = st.session_state.context_manager
    summarizer: ConversationSummarizer = st.session_state.summarizer
    budgeter: TokenBudgeter = st.session_state.token_budgeter
    ...
    if submitted_query:
        ...
        if cache_key in st.session_state.response_cache:
            ...
            if cached_query == normalized_query:
                ...
                turn_index = len(st.session_state.conversation_history)
                mem_store.append_message(st.session_state.session_id, "user", query, turn_index - 1)
                mem_store.append_message(
                    st.session_state.session_id, "assistant", display_response, turn_index
                )
                mem_store.log_metric(
                    session_id=st.session_state.session_id,
                    tokens_in=max(1, len(query) // 4),
                    tokens_out=max(1, len(display_response) // 4),
                    latency_ms=0,
                    summary_used=False,
                    rag_docs_used=0,
        ...
        rag: Optional[RAGKnowledgeLayer] = None
        try:
            rag = get_rag_layer()
        except Exception as e:
            rag = None
            log_pipeline_error(f"RAG load error; continuing without retrieval: {e}")

        # Cache RAG status in session_state so sidebar can show it
        if rag is None:
            st.session_state.rag_is_ready = False
            st.session_state.rag_error = "RAG load error"
        else:
        ...
        rag_context = ""
        persisted_history = mem_store.get_messages(st.session_state.session_id, limit=200)
        live_history = st.session_state.conversation_history or []
        combined_history = persisted_history if persisted_history else live_history
        existing_summary = mem_store.get_summary(st.session_state.session_id)
        existing_memory = mem_store.get_user_memory(st.session_state.session_id)
        if not existing_memory:
            ...
            mem_store.upsert_user_memory(st.session_state.session_id, existing_memory)
        ...
        conversation_context = ContextManager.recent_turns_text(ctx.recent_turns)
        use_conversation_context = bool(conversation_context) and bool(ctx.linked_followup)
        ...
        if rag is not None and rag.is_ready:
            ...
            rcache_key = retrieval_manager.make_cache_key(age_category, retrieval_query)
            cached_docs = mem_store.get_retrieval_cache(rcache_key)
            if cached_docs is not None:
                rag_matches = cached_docs
            else:
                raw_matches = rag.search(
                    retrieval_query,
                    top_k=max(8, retrieval_top_k),
                    age_category=age_category,
                )
                rag_matches = retrieval_manager.rerank_dedupe(retrieval_query, raw_matches, keep=5)
                mem_store.upsert_retrieval_cache(rcache_key, rag_matches)
        ...
        evidence_block = retrieval_manager.evidence_block(rag_matches, max_items=5)
        safety_rules = (
            "No tobacco acquisition instructions for minors.\n"
            "Do not provide unsafe medical claims.\n"
            "No citations dump or raw retrieval fragments."
        )
        sections = {
            "system_prompt": MEDICAL_PREFIX.strip(),
            "safety_rules": safety_rules,
            "user_query": query,
            "recent_turns": conversation_context if use_conversation_context else "",
            "summary": ctx.summary_text,
            "memory": ctx.memory_text,
            "rag_evidence": evidence_block or "No relevant evidence retrieved.",
        ...
        system_prompt = build_shs_structured_prompt(
            query=query,
            evidence_block=budget.sections["rag_evidence"],
            age_tier=age_tier,
            conversation_context=budget.sections["recent_turns"],
        )
        ...
        mem_store.append_message(st.session_state.session_id, "user", query, turn_index - 1)
        mem_store.append_message(
            st.session_state.session_id, "assistant", current_response, turn_index
        )
        history_for_summary = mem_store.get_messages(st.session_state.session_id, limit=250)
        should_refresh_summary = summarizer.should_refresh(
            total_messages=len(history_for_summary),
            last_summary_at=st.session_state.summary_last_message_count,
            overflow=budget.trimmed_context,
        )
        if should_refresh_summary:
            updated_summary = summarizer.update(existing_summary, history_for_summary)
            mem_store.upsert_summary(st.session_state.session_id, updated_summary)
            st.session_state.summary_last_message_count = len(history_for_summary)
        mem_store.log_metric(
            session_id=st.session_state.session_id,
            tokens_in=budget.tokens_in,
            tokens_out=max(1, len(current_response) // 4),
            latency_ms=0,
            summary_used=bool(ctx.summary_text and ctx.summary_text != "{}"),
            rag_docs_used=len(rag_matches),
